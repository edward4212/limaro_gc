        ...
        <table class="table table-hover table-sm datatable datatable-export mb-0"
            ...
            <tr>
                ...
                <td style="max-width:200px;">
                    ...
                    <span style="font-size:11px;background:#f1f5f9;padding:1px 4px;border-radius:3px;">
                        <?= e($s['codigo_documento']) ?>
                    </span><br>
                    <?php endif; ?>
                    <small style="font-size:11px;display:block;white-space:normal;overflow:hidden;
                                  text-overflow:ellipsis;max-width:190px;color:#6b7280;">
                        <?= e($s['solicitud'] ?? '') ?>
                    </small>
                </td>
                <td style="font-size:11px;"><?= labelTipoSolicitud($s['tipo_solicitud'] ?? '') ?></td>
                <td class="text-center">
                    <?php $nAnexos = (int)($s['total_anexos'] ?? 0); ?>
                    <?php if ($nAnexos > 0): ?>
                    <span class="badge bg-success" style="font-size:10px;cursor:default;"
                          title="<?= $nAnexos ?> archivo(s) adjunto(s)">
                        <i class="bi bi-paperclip me-1"></i><?= $nAnexos ?>
                    </span>
                    <?php else: ?>
                    <span class="text-muted" style="font-size:11px;" title="Sin archivos anexos">
                ...
                <td style="font-size:11px;white-space:normal;overflow:hidden;text-overflow:ellipsis;max-width:130px;">
                    <?= e($s['solicitante'] ?? '—') ?>
                </td>
                <td style="font-size:11px;white-space:normal;overflow:hidden;text-overflow:ellipsis;max-width:130px;">
                    ...
                    <?php elseif (!empty($s['nombre_asignado'] ?? '')): ?>
                        <?= e($s['nombre_asignado']) ?>
                    <?php else: ?>
                        <em class="text-muted" style="font-size:11px;">Sin asignar</em>
                    <?php endif; ?>
                </td>
                <td class="text-center">
