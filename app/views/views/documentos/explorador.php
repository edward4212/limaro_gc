        ...
        subprocesos.forEach(s => {
            ...
            <div class="col-6 col-md-4">
                <div class="card card-tipo text-center py-3"
                    ...
                    <code style="font-size:12px;">${esc(s.sigla_subproceso)}</span>
                </div>
            </div>`;
        });
        html += '</div>';
...
function renderDocumentos(docs, pagina) {
    ...
    docsPag.forEach(d => {
        ...
        html += `<tr>
            <td><code style="font-size:12px;background:#f1f5f9;padding:2px 5px;border-radius:3px;">${esc(d.codigo)}</span></td>
            <td>
