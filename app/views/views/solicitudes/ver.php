<!-- HU-027: Modal Finalizar sin Trámite -->
<div class="modal fade" id="modalFinalizarST" tabindex="-1">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <form id="formFinalizarST"
              action="<?= e(APP_URL) ?>/solicitudes/finalizar-sin-tramite/<?= $sol['id_solicitud'] ?>"
              method="POST">
          <?= csrfField() ?>
          <div class="mb-1">
            ...
            <textarea name="motivo" class="form-control" rows="3"
                      placeholder="Explique por qué se finaliza sin trámite..."
                      id="motivoFinalizarST"></textarea>
          </div>
        </form>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary btn-sm" data-bs-dismiss="modal">Cancelar</button>
        <button type="button" class="btn btn-danger btn-sm" onclick="submitFinalizarST()">
...
<script>
function submitFinalizarST() {
    var m = document.getElementById('motivoFinalizarST').value.trim();
    if (m.length < 10) {
        Swal.fire({ icon:'warning', title:'Motivo requerido',
            text:'El motivo debe tener al menos 10 caracteres.', confirmButtonColor:'#1B3A6B' });
        return;
    }
    document.getElementById('formFinalizarST').submit();
}
</script>
        ...
        <div class="card mb-3">
            ...
            <div class="card-body">
                <div class="row mb-2">
                    ...
                    <div class="col-sm-8"><?= labelTipoSolicitud($sol['tipo_solicitud'] ?? '') ?></div>
                </div>
                <div class="row mb-2">
                ...
                <?php
                // HU-023: no mostrar documento en CREACION (aún no existe)
                $tipoSol = strtoupper($sol['tipo_solicitud'] ?? '');
                $esCrea  = $tipoSol === 'CREACION';
                ?>
                <?php if (!$esCrea && ($sol['codigo_documento'] ?? null)): ?>
                <div class="row mb-2">
                ...
                <?php endif; ?>
                <?php if (!$esCrea && ($sol['nombre_documento'] ?? null)): ?>
                <div class="row mb-2">
                    <div class="col-sm-4 fw-bold text-muted">Nombre Documento:</div>
                    <div class="col-sm-8"><?= e($sol['nombre_documento']) ?></div>
                </div>
                <?php endif; ?>
                <div class="row mb-2">
    ...
    <div class="col-lg-4">
        <div class="card">
            ...
            <div class="card-footer">
                <?php $esFinalizada = in_array($sol['estado_solicitud'], ['FINALIZADA','CERRADA','FINALIZADA_SIN_TRAMITE']); ?>
                <?php if ($esFinalizada): ?>
                <!-- CA-1/CA-2: solicitud finalizada — sin nuevos comentarios -->
                <div class="alert alert-secondary py-2 mb-0 text-center" style="font-size:12px;">
                    <i class="bi bi-lock me-1"></i>
                    Solicitud <strong>cerrada</strong> — no se pueden agregar comentarios.
                </div>
                <?php else: ?>
        ...
        <!-- HU-027 / HU-N04: Botón finalizar sin trámite — ocultar si tiene tarea asignada -->
        <?php if (
            $sol['estado_solicitud'] !== 'FINALIZADA_SIN_TRAMITE' &&
            $sol['estado_solicitud'] !== 'FINALIZADA' &&
            empty($tieneTarea) &&
            (Auth::hasRole([1, 'ADMINISTRADOR', 'Administrador del Sistema']) || Auth::puede('solicitudes_gestion','editar'))
        ): ?>
        <div class="mt-2">
            <button type="button" class="btn btn-outline-danger btn-sm w-100"
                    data-bs-toggle="modal" data-bs-target="#modalFinalizarST">
                <i class="bi bi-slash-circle me-1"></i>Finalizar sin Trámite
            </button>
        </div>
        <?php endif; ?>
        ...
        <!-- Asignar si es CREADA, tiene permisos y NO está finalizada sin trámite -->
        <?php if ($sol['estado_solicitud'] === 'CREADA' && Auth::puede('sol_radicadas', 'editar') && $sol['estado_solicitud'] !== 'FINALIZADA_SIN_TRAMITE'): ?>
        <div class="card mt-3">
